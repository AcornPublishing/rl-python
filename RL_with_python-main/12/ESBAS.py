...
def ESBAS(env_name, hidden_sizes=[32], lr=1e-2, num_epochs=2000, buffer_size=100000, discount=0.99, render_cycle=100, update_target_net=1000, 
    ...
    file_writer = tf.summary.create_file_writer(LOG_DIR+'/ESBAS_'+clock_time+'_'+hyp_str, tf.get_default_graph())
    ...
    with file_writer.as_default():
        for ep in range(num_epochs):
            ...
            while step_count < 2**beta:
                ...
                tf.summary.scalar(tag='algorithm_selected', simple_value=best_dqn, step = step_count)
                ...
                tf.summary.scalar(tag='train_performance', simple_value=np.mean(ep_rew), step = step_count)                
