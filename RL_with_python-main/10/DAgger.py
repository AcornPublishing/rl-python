...
dirnm='D:/model/python/99_Reinforcement-Learning-Algorithms-with-Python-master/Chapter10/expert/'
def expert():
    '''
    ...
    saver = tf.train.import_meta_graph(dirnm + 'model.ckpt.meta')
    saver.restore(sess_expert,tf.train.latest_checkpoint(dirnm))   #'expert/'))
    ...
    for it in range(dagger_iterations):
        ...
        for _ in range(train_epochs):
            ...
            for b in range(n_batches):
                ...
                tr_loss = sess.run([p_loss], feed_dict={
                                obs_ph:shuffled_X[p_start:p_start+batch_size], 
                                act_ph:shuffled_y[p_start:p_start+batch_size]})

                sess.run(p_opt, feed_dict={
                                obs_ph:shuffled_X[p_start:p_start+batch_size], 
                                act_ph:shuffled_y[p_start:p_start+batch_size]})
